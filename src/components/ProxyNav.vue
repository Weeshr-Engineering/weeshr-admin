...
const canShow = computed(() => {
    return superAdminStore.isProxy && superAdminStore.isVendor
})
const endProxy = ()=>{
  useSuperAdminStore().setVendor(false, '')
  localStorage.setItem('vendor', JSON.stringify(''))
  sessionStorage.setItem('isProxy', JSON.stringify(false))
  router.push('/user')
}
</script>

<template>
    <div v-if="canShow" class="fixed top-0 right-0 z-50 w-screen flex items-start justify-center">
        <AlertDialog>
            <AlertDialogTrigger asChild>
                <div class="border-2 rounded-b-3xl cursor-pointer bg-white w-52 text-blue-700 flex items-center justify-center gap-4 py-2 shadow-lg">
            ...
            <AlertDialogContent>
                ...
                <AlertDialogDescription>
                    Do you want to end PROXY ACTIONS?
                </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction @click="()=> endProxy()">Continue</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    </div>
